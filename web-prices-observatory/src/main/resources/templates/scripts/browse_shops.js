var GetShopsCallback = function (data) {    
    ...
    for (var p of data.shops) {
        tagstring = p.tags.join();
        outputContent += ' \
            <tr> \
                <th scope="col">' + p.id + '</th> \
                <th scope="col">' + p.name + '</th> \
                <th scope="col">' + p.address + '</th> \
                <th scope="col">' + tagstring + '</th> \
                <th scope="col">' + p.withdrawn + '</th> \
                <th scope="col"><a class="nav-link" href="/show_shop?id=' + p.id + '">Λεπτομέρειες</a>' + '</th> \
            </tr> \
        ';
    }
    ...
    outputContent += ' \
        <div> \
            <ul class="pagination"> \
                <li class="page-item"><a class="nav-link disabled">Συνολικά Καταστήματα: '+ data.total + '</a></li> \
    ';  
    if ([[${start}]] == 0) {
    ...
    outputContent += '<a class="page-link" href="/browse_shops?start='
...
$(document).ready(function() {
    ...
    RestGet ("/shops", query, GetShopsCallback);

    $("#BrowseShopsForm").submit(function(e) {
        e.preventDefault();
        ...
        location.href = "/browse_shops" + suffix;
